...
def generate_barcode_labels():
    """
    API endpoint to generate QR code labels for Multi GRN line items
    Supports serial, batch, and non-managed items
    Returns: JSON with label data including QR code images
    """
    try:
        ...
        data = request.get_json()
        line_selection_id = data.get('line_selection_id')
        label_type = data.get('label_type', 'batch')  # 'serial', 'batch', or 'standard'
        
        if not line_selection_id:
            return jsonify({
                'success': False,
                'error': 'Missing required parameter: line_selection_id'
            }), 400
        ...
        if label_type == 'serial':
            ...
            # Get number of packs from the first serial
            first_serial = serial_details[0]
            num_packs = first_serial.no_of_packs if first_serial.no_of_packs else total_serials
            ...
            for pack_idx in range(1, num_packs + 1):
                ...
                ref_serial = pack_serials[0]
                serial_list = ', '.join([s.serial_number for s in pack_serials])
                grn_number = ref_serial.grn_number or f"{batch_number}/{line_selection.id}"
                
                # Format matching user's requested JSON structure
                qr_data = {
                    "id": grn_number,
                    "po": po_number,
                    "item": line_selection.item_code,
                    "batch": serial_list[:50] + "..." if len(serial_list) > 50 else serial_list,
                    "qty": float(ref_serial.qty_per_pack) if ref_serial.qty_per_pack else 1,
                    "pack": f"{pack_idx} of {num_packs}",
                    "grn_date": grn_date,
                    "exp_date": ref_serial.expiry_date.strftime('%Y-%m-%d') if ref_serial.expiry_date else "N/A"
                }
                ...
                label = {
                    ...
                    'po_number': po_number,
                    'serial_number': serial_list,
                    'quantity': float(ref_serial.qty_per_pack) if ref_serial.qty_per_pack else 1,
                    'grn_date': grn_date,
                    'grn_number': grn_number,
                    'expiration_date': ref_serial.expiry_date.strftime('%Y-%m-%d') if ref_serial.expiry_date else 'N/A',
                    'item_code': line_selection.item_code,
            ...
            if not batch_details:
                return jsonify({
                    'success': False,
                    'error': 'No batch details found for this item'
                }), 400
            ...
            for batch_detail in batch_details:
                ...
                for pack_idx in range(1, num_packs + 1):
                    grn_number = batch_detail.grn_number or f"{batch_number}/{line_selection.id}-{pack_idx}"
        ...
        else:  # Standard/non-managed items
            non_managed_details = line_selection.non_managed_details
            
            if non_managed_details and len(non_managed_details) > 0:
                total_packs = len(non_managed_details)
                
                for idx, non_managed in enumerate(non_managed_details, start=1):
                    grn_number = non_managed.grn_number or f"{batch_number}/{line_selection.id}-{idx}"
                    ...
                    qr_data = {
                        ...
                        "grn_date": grn_date,
                        "exp_date": non_managed.expiry_date if non_managed.expiry_date else "N/A"
                    }
                    ...
                    label = {
                        ...
                        'grn_date': grn_date,
                        'grn_number': grn_number,
                        'expiration_date': non_managed.expiry_date if non_managed.expiry_date else 'N/A',
                        'item_code': line_selection.item_code,
                        'item_name': line_selection.item_description or '',
                        'qr_code_image': qr_code_image,
                        'qr_data': qr_data
                ...
                # Fallback: Generate a single label
                grn_number = f"{batch_number}/{line_selection.id}"
                ...
                qr_data = {
                    ...
                    "pack": "1 of 1",
                    "grn_date": grn_date,
                    "exp_date": line_selection.expiry_date.strftime('%Y-%m-%d') if line_selection.expiry_date else "N/A"
                }
                ...
                label = {
                    ...
                    'grn_date': grn_date,
                    'grn_number': grn_number,
                    'expiration_date': line_selection.expiry_date.strftime('%Y-%m-%d') if line_selection.expiry_date else 'N/A',
                    'item_code': line_selection.item_code,
                    'item_name': line_selection.item_description or '',
                    'qr_code_image': qr_code_image,
                    'qr_data': qr_data
        ...
        logging.error(f"Error generating QR labels for Multi GRN: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
