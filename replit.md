...
### 2025-11-07
*   **Multi GRN Module Enhancement - Batch Entry & QR Labels**: Enhanced Multi GRN module with comprehensive batch/serial number entry system similar to GRPO. Added new database models (MultiGRNBatchDetails, MultiGRNSerialDetails) to track detailed batch and serial information for each line item. Implemented API endpoints for batch/serial management (`/api/line-selections/<id>/batch-details`, `/api/line-selections/<id>/serial-details`) and automatic QR code label generation (`/api/line-selections/<id>/generate-qr-labels`). Updated SAP B1 posting logic to build BatchNumbers and SerialNumbers arrays from the detail models, supporting expiry dates, manufacturer serial numbers, and pack quantities. Created MySQL migration file (`mysql_multi_grn_batch_serial_details.sql`) for schema changes. The module now supports the same workflow as GRPO: item validation → batch/serial entry → QR label generation → SAP B1 posting with full batch/serial tracking.
